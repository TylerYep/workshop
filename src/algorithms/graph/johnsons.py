...
from src.algorithms import bellman_ford_shortest_paths, dijkstra_shortest_paths
...
def johnsons_shortest_paths(graph: Graph[V]) -> Dict[V, Dict[V, float]]:
    ...
    efficient than the Floyd-Warshall all-pairs shortest paths algorithm when
    run on sparse graphs.

    Runtime: O(|V||E| + |V|^2 log |V|)
    """
    # Create augmented graph G' that will be fed into the Bellman-Ford step by
    # copying the graph and adding an extra node. We convert the graph into a
    # Node[Tuple[V]] type in order to ensure we can add a new, unique node.
    ...
    result: Dict[V, Dict[V, float]] = {}
    for u in graph:
        costs = dijkstra_shortest_paths(reweighted_graph, u)
        result[u] = {
            v: cost + bellman_costs[v] - bellman_costs[u] for v, cost in costs.items()
        }
    return result
